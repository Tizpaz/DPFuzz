...
parser.add_argument("--name", help='The name of benchmark')
...
#            self.DuplicateByte
    ...
    def create_candidate(self, mutate = True, cur_act = 0, e = 0.05, m_p = 0.1, w_p = 0.1):
        a_index = -1
        if mutate:
            if random.random() < 1 - e and len(self.cluster_paths.keys()) >= NUM_CLUSTERS:
                cluster = random.choice(self.cluster_paths.keys())
                if random.random() < w_p:
                    cluster_worst_costs = []
                    for x in self.cluster_paths[cluster]:
                        cluster_worst_costs.append(self.worst_costs[x])
                    sum_costs = np.sum(cluster_worst_costs)
                    prob = [float(x/sum_costs) for x in cluster_worst_costs]
                    if np.sum(prob) > 0.999:
                        key = np.random.choice(self.cluster_paths[cluster],p=prob)
                    else:
                        key = np.random.choice(self.cluster_paths[cluster])
                else:
                    max_cost = -1
                    key = np.random.choice(self.cluster_paths[cluster])
                    for x in self.cluster_paths[cluster]:
                        if self.worst_costs[x] > max_cost:
                            max_cost = self.worst_costs[x]
                            key = x
            else:
                key = random.choice(self.population.keys())
...
class MutationCoverageFuzzer(MutationFuzzer):
    ...
    def run(self, runner):
        ...
        if is_interesting:
            ...
            return ""
        elif not key_path in self.path_cluster.keys() and self.num_inp - self.last_update[key_path] > STEPS_TO_KILL and len(self.population) > MIN_NUM_PATH and self.worst_costs[key_path] <= np.percentile(self.worst_costs.values(),50):
        ...
        elif key_path in self.path_cluster.keys() and ALLOWED_REMOVE_CLUSTER_PATH and self.num_inp - self.last_update[key_path] > STEPS_TO_KILL and len(self.cluster_paths[self.path_cluster[key_path]]) > MIN_NUM_PATH_PER_CLUST:
    ...
    if INPUT_SIZE:
        ...
        for key in mutation_fuzzer.coverages_seen.keys():
            ...
            if key in mutation_fuzzer.model_fit.keys():
                ...
                print("The cluster is: " + str(mutation_fuzzer.path_cluster[key]))
            else:
                print("The path key is: " + str(key))
...
if __name__ == "__main__":
    # seed_inputs = ["1 3","0 10 5 11","2 15 20 3 1"]
    # seed_inputs = ["1 2","10 0","64 2 3"]
    # for prim MST
    # seed_inputs = ["0 1 3 1 2 2 2 0 1","0 1 2 1 2 3 2 3 6","0 1 3 1 3 5 2 3 4 1 2 10"]
    # # for Boyer_Moore
    # seed_inputs = ["123ABC 123,123", "ABAA,AA", "123456,456"]
    # seed_inputs = ["1 3","0 2 2 5","2 5 7 8 10","2 5 7 8 10 12 15 18"]
