...
if "Logistic_Regression" in filename:
    for x in df.columns:
        if x == 'weight':
            pass
        elif x.startswith('optimize'):
            if 'while' in x or 'if' in x:
                pass
            else:
                header_to_drop.append(x)
        else:
            header_to_drop.append(x)

elif "make_multi_classification" in filename:
    for x in df.columns:
        if x == 'weight':
            pass
        elif x.startswith('samples_generator'):
            if 'while' in x or 'if' in x or 'for' in x:
                pass
            else:
                header_to_drop.append(x)
        else:
            header_to_drop.append(x)
elif "Gaussian_Process" in filename:
    for x in df.columns:
        if x == 'weight':
            pass
        elif x.startswith('lbfgsb'):
            if 'while' in x or 'for' in x:
                pass
            else:
                header_to_drop.append(x)
        else:
    ...
    for x in df.columns:
        if x == 'weight':
            pass
        elif 'while' in x or 'if' in x or 'for' in x:
                pass
        else:
            header_to_drop.append(x)


df = df.drop(header_to_drop,1)
...
for x in X.T:
    for e in x:
        if "\'" in str(e) or "False" in str(e) or "True" in str(e):
            flag_contains = True
    if flag_contains:
        print(sorted(set(x)))
        le_fit = le.fit(x)
...
for i in range(5):
    ...
    for train_index, test_index in kf.split(X, y,id):
        ...
        clf_temp = DecisionTreeClassifier(criterion=criterion,splitter='best',max_depth=max_depth_tree_num,min_samples_split = 2)
